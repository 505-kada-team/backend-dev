...
const getMenu = asyncHandler(async (req, res) => {
  ...
  return new ApiResponse(200, menus, 'Daftar menu berhasil diambil').send(res);
});

const getMenuById = asyncHandler(async (req, res) => {
  const menu = await menuService.getMenuById(req.params.id, req.user._id);
  return new ApiResponse(200, menu, 'Detail menu berhasil diambil').send(res);
});
...
const updateMenu = asyncHandler(async (req, res) => {
  ...
  return new ApiResponse(200, menu, 'Menu berhasil diperbarui').send(res);
});

const deleteMenu = asyncHandler(async (req, res) => {
  await menuService.deleteMenu(req.params.id, req.user._id);
  return new ApiResponse(200, null, 'Menu berhasil dihapus').send(res);
});
